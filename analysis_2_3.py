...
def get_historical_win_prob(exclude_recent=100):
    """ 从数据库获取赔率对应的历史胜率，排除最近的比赛 """
    ...
    cursor.execute("""
        ...
        AND b.id NOT IN (SELECT id FROM bets ORDER BY id DESC LIMIT ?)  -- 排除最近的比赛
        GROUP BY bo.odds
...
if __name__ == "__main__":
    calculate_profit(amount=100000, recent=150, debug=True)
